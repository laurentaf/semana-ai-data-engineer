"""Backfill historical dates for orders and ingest reviews into Qdrant."""
...
while current <= end_date:
    ...
    months.append((y, m, target, current, days))
    current = next_m

# Assign orders to months proportionally
order_idx = 0
pairs = []
for y, m, target, month_start, days_in_month in months:
    count = min(target, total - order_idx)
    for _ in range(count):
        day = random.randint(1, days_in_month)
        hour = random.randint(8, 22)
        minute = random.randint(0, 59)
        dt = datetime(y, m, day, hour, minute)
        pairs.append((all_ids[order_idx], dt.isoformat()))
        order_idx += 1
        if order_idx >= total:
            break
    if order_idx >= total:
...
print(f"Assigning dates to {len(pairs)} orders...")
...
for i in range(0, len(pairs), BATCH):
    ...
    values = ",".join(
        cur.mogrify("(%s::uuid, %s::timestamptz)", (oid, dt)).decode()
        for oid, dt in batch
    )
    cur.execute(f"UPDATE orders SET created_at = v.dt FROM (VALUES {values}) AS v(oid, dt) WHERE orders.order_id = v.oid")
    conn.commit()
    if (i + BATCH) % 2000 == 0:
        print(f"  Updated {i + BATCH}/{len(pairs)}")

print("Dates backfilled!")
...
# Get orders with customer state for review payload
cur.execute("""
...
sample_size = min(int(len(all_orders) * 0.15), 10000)
...
for order_id, created_at, product_id, customer_state in selected:
    ...
    if len(points) >= BATCH_SIZE:
        ...
        print(f"  Reviews upserted: {total_upserted}", end="\r")
        points = []
...
print(f"\nDone! {total_upserted} reviews upserted to Qdrant.")
